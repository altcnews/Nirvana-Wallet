...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.Nirvana.Nirvana scientistwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Nirvana Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("NirvanaWallet", appPath);
    } else {
      autorunSettings.remove("NirvanaWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\Nirvana", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:Nirvana");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\Nirvana\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\Nirvana\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("Nirvanawallet.cfg"));
